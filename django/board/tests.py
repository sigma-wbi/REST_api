...
class Testboard(APITestCase):
    ...
    def setUp(self):
        ...
        self.question_url2 = f"/api/board/question/"
...
# --------------put, delete는 상세페이지---------------------
    ...
    def test_detail_board_update(self): # 비 로그인시 상세페이지에서 수정
        ...
        self.response = self.client.put(f'{self.question_url2}{self.question.id}/', data, format='json')
        self.assertEqual(self.response.status_code, status.HTTP_200_OK, msg="비로그인시 상세페이지 수정되면 안됨")
    ...
    def test_detail_board_delete_login(self): # 로그인시 상세페이지에서 삭제
        ...
        self.response = self.client.delete(f'{self.question_url2}{self.question.id}/', data, format='json')
